...
*  Is there a significance difference in the body weight between the two habitats?
*  Is there a significance difference in the body length between the two habitats?
...
```{r, numSNPs}
indNames(genlight.data) #individual names instead of genlight.data@ind.names
...
We calculate the Fst values from this reduced set of SNPs with the ‘StAMPP’ package:
...
We can specify our populations:
...
Then we can apply the Hs function on these data to obtain the average heterozygosity for each population:
...
We can perform a PCA directly on our genlight data
...
* No significant difference in body length/weight between habitats, although brackish a bit heavier.

* All Fst<0.15, although the L10 population is way more different than the rest (high Fst even when compared to same habitat populations).

* Heterozygosity pretty uniform across all population, population L10 a bit lower heterozygosity in comparison to the rest of the populations.

* Clustering from neighbor-joining tree and PCA are similar, with the L10 population being very different genetically from the rest of the populations.
